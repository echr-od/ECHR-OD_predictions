...
def load_ECHR_instance(X_file, y_file, min_threshold):
...
def generate_datasets_descriptors():
    import itertools
    ...
    ] #['BoW', 'descriptive', 'descriptive+BoW']
    keys = ['name', 'features', 'labels', 'min_threshold']

    datasets = []
    for i in itertools.product(articles, flavours):
        base_path = 'data/input/article_{}/'.format(i[0])
        descriptor = {
            'name': 'Article {} - {}'.format(i[0], i[1][1]),
